...
func (privKey *privateKey) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	pubKeyType := backend.GetPublicKeyAlgorithm(privKey)
	signAlgo := x509.UnknownSignatureAlgorithm

	// Determine the signature algorithm using the hash function and key type. Exclude
	// RSAPSS algorithms
	for algo, details := range backend.SignatureAlgorithmDetails {
		if details.PubKeyAlgo == pubKeyType && details.Hash == opts.HashFunc() && !backend.IsRSAPSS(algo) {
			signAlgo = algo
			break
		}
	}
	...
	return backend.FormatSignature(privKey, response.Signature), nil
}
