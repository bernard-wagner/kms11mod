...
func convertJSONWebKeyToObject(client keyvault.BaseClient, key keyvault.JSONWebKey, addCert bool) ([]kms11.Object, error) {
	...
	privObject, err := kms11.NewPrivateKeyObject([]byte(keyVersion(key)), *key.Kid, privKey, encrypt, decrypt, sign)
	if err != nil {
		return nil, err
	}

	pubObject, err := kms11.NewPublicKeyObject([]byte(keyVersion(key)), *key.Kid, privKey.Public(), encrypt, decrypt, sign)
	if err != nil {
		return nil, err
	}

	var certObject kms11.Object
	if addCert {
		...
		certObject, err = kms11.NewCertificateObject([]byte(keyVersion(key)), *key.Kid, cert, true)
		if err != nil {
			return nil, err
		}
	}
...
func convertCertificateBundleToObject(client keyvault.BaseClient, bundle keyvault.CertificateBundle, key keyvault.JSONWebKey) ([]kms11.Object, error) {
	...
	certObject, err := kms11.NewCertificateObject([]byte(*bundle.ID), *bundle.ID, []*x509.Certificate{cert}, true)
	if err != nil {
		return nil, err
	}
