...
func (privKey *privateKey) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	pubKeyAlgorithm := kms11.GetPublicKeyAlgorithm(privKey)

	for kvSigAlgo, sigAlgoDetails := range signatureAlgorithmProperties {
		if sigAlgoDetails.PubKeyAlgo == pubKeyAlgorithm && kms11.SignatureAlgorithmDetails[sigAlgoDetails.SignatureAlgorithm].Hash == opts.HashFunc() {
			signAlgo = kvSigAlgo
			break
		}
	}
